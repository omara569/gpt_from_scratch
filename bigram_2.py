...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        # We don't have a single attention of head size 32
        ## Instead of 1 communication channel we have 4 communication channels in parallel and each one of them will be smaller correspondingly
    ...
    def forward(self, idx, targets=None):
        ...
        #x = self.sa_heads(x) # We feed into the self attention head once we've combined the token_embeddings and the positional embeddings
        #x = self.ffwd(x) # B, T, C --> Note that the FeedForward is performed on a per token level
        x = self.blocks(x)
